...
exports.book_create_post = [
  ...
  // Validates every genre array entries.
  body("genre.*").escape(),
...
exports.book_delete_post = asyncHandler(async (req, res, next) => {
  ...
  if (bookInstances.length > 0) {
    // There are book instances to be deleted first.
    res.render("bookDelete", {
      title: "Delete Book",
      book: book,
      bookInstances: bookInstances,
    ...
    // There are no book instances to be deleted.
...
exports.book_update_get = asyncHandler(async (req, res, next) => {
  console.log("Book ID:", req.params.id); // Log the book ID
...
exports.book_update_post = [
  ...
  body("isbn", "ISBN must not be empty").trim().isLength({ min: 1 }).escape(),
  body("genre.*").escape(),
  // genre.* Applies escape() to all values in genre field array (Works also for objects)
  ...
  asyncHandler(async (req, res, next) => {
    ...
    const book = new Book({
      ...
      isbn: req.body.isbn,
      genre: req.body.genre,
      _id: req.params.id, // This is required! If missing new ID will be assigned!
    });
    if (!errors.isEmpty()) {
